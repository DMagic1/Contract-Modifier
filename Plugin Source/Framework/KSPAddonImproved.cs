...
namespace ContractModifier.Framework
{
	...
	internal class DMCM_KSPAddonImproved : Attribute
	{
		[Flags]
		public enum Startup
		{
		...
		public DMCM_KSPAddonImproved(Startup mask, bool once = false)
		{
			runOnce = once;
			scenes = mask;
		}
	...
	internal class CustomAddonLoader : MonoBehaviour
	{
		...
		class AddonInfo
		{
			...
			public readonly DMCM_KSPAddonImproved addon;
			public bool created;

			internal AddonInfo(Type t, DMCM_KSPAddonImproved add)
			{
				type = t;
				created = false;
			...
			internal DMCM_KSPAddonImproved.Startup Scenes
			{
				get
				{
					return addon.scenes;
		...
		void Awake()
		{
			...
			{
				var attr = ((DMCM_KSPAddonImproved[])ourType.GetCustomAttributes(typeof(DMCM_KSPAddonImproved), true)).SingleOrDefault();
				if (attr != null)
				{
		...
		void OnLevelWasLoaded(int level)
		{
			...
			DMCM_KSPAddonImproved.Startup mask = 0;
			...
			switch (scene)
			{
				case GameScenes.EDITOR:
					mask = DMCM_KSPAddonImproved.Startup.EditorVAB;
					break;

				case GameScenes.CREDITS:
					mask = DMCM_KSPAddonImproved.Startup.Credits;
					break;

				case GameScenes.FLIGHT:
					mask = DMCM_KSPAddonImproved.Startup.Flight;
					break;

				case GameScenes.LOADING:
					mask = DMCM_KSPAddonImproved.Startup.Instantly;
					break;

				case GameScenes.MAINMENU:
					mask = DMCM_KSPAddonImproved.Startup.MainMenu;
					break;

				case GameScenes.SETTINGS:
					mask = DMCM_KSPAddonImproved.Startup.Settings;
					break;

				case GameScenes.SPACECENTER:
					mask = DMCM_KSPAddonImproved.Startup.SpaceCenter;
					break;

				case GameScenes.TRACKSTATION:
					mask = DMCM_KSPAddonImproved.Startup.TrackingStation;
					break;

				case GameScenes.PSYSTEM:
					mask = DMCM_KSPAddonImproved.Startup.PSystemSpawn;
					break;
