...
namespace ContractModifier.Framework
{
    ...
    public abstract class DMCM_MBW : DMCM_MBE
    {
		...
		internal void DrawGUI()
		{
			...
			GUI.skin = DMCM_SkinsLibrary.CurrentSkin;
        ...
        private void DrawToolTip()
        {
            ...
            if (TooltipsEnabled && Visible && (strToolTipText != "") && ((TooltipDisplayForSecs == 0) || (fltTooltipTime < (Single)TooltipDisplayForSecs)))
            {
                ...
                GUIStyle styleTooltip = DMCM_SkinsLibrary.CurrentTooltip;
                ...
                if (TooltipMaxWidth > 0)
                {
                    ...
                    DMCM_SkinsLibrary.CurrentTooltip.CalcMinMaxWidth(contTooltip, out minwidth, out maxwidth); // figure out how wide one line would be
                    _TooltipPosition.width = Math.Min(TooltipMaxWidth - DMCM_SkinsLibrary.CurrentTooltip.padding.horizontal, maxwidth); //then work out the height with a max width
                    _TooltipPosition.height = DMCM_SkinsLibrary.CurrentTooltip.CalcHeight(contTooltip, TooltipPosition.width); // heres the result
                }
                else
                {
                    //calc the new width and height
                    Vector2 Size = DMCM_SkinsLibrary.CurrentTooltip.CalcSize(contTooltip);
